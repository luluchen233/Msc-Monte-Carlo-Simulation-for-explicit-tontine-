...
def wealth_monte_carlo():
    ...
    age = df.loc[df['id']==1,'age0'].values[0]
    ...
    for year in range(50):
        ...
        if df.loc[df['id']==1,'money_total'].empty==False:
            ...
            wealth_track.append(df.loc[df['id']==1,'money_total'].values[0])
        #mortality rate
        for i in range(num_of_people):
            #age now = age now + 1
            df.iloc[i,2] += 1
    ...
    return wealth_track, int(age), df

for run in range(1):
    wealth, age, df = wealth_monte_carlo()
    plt.plot(list(range(age, age+len(wealth))), wealth)
    
plt.xlabel("Age")
plt.ylabel("Total money")  
plt.title('Monte Carlo Analysis for wealth')
