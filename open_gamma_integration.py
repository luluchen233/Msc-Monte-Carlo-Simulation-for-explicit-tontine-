...
utontine_total = []
ubequest_total = []
gamma_range = [-999,-3,-1,0,0.5,0.6,0.7,0.8,0.9]
runtime = 1
num_of_people = 500
np.random.seed(30)
...
#        ax.plot(list(range(50, 50+len(wealth_track))), np.array(wealth_track) * np.array(ct_track), label='$\gamma=$'+str(gamma))
        ax.plot(list(range(50, 50+len(wealth_track))), np.array(wealth_track), label='$\gamma=$'+str(gamma))
...
#plt.title('The optimal monetary consumption ($\gamma$)')
plt.title('The life long wealth for open tontine group ($\gamma$)')
