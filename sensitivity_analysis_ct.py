...
#            df.iloc[i,3] = 1
    ...
    C=np.array([[1.10447455646221, 1.1123275011739901]]*num_of_people)
...
#        df.iloc[i,4] = at
    ...
    ubequest = [df.loc[df['id']==1, 'bequest_motive'].values[0] * df.loc[df['id']==1, 'mortality rate'].values[0] * 
...
#                ct_track.append(df.loc[df['id']==i,'optimal_consumption'].values[0])
        for i in range(1,2):        
            if df.loc[df['id']==1,'money_total'].empty==False:
                ...
                ubequest.append([df.loc[df['id']==1, 'bequest_motive'].values[0] * df.loc[df['id']==1, 'mortality rate'].values[0] * 
                            np.log((1-df.loc[df['id']==1, 'tontine proportion'].values[0])*df.loc[df['id']==1, 'money_total'].values[0])])
#                ct_track.append(df.loc[df['id']==i,'optimal_consumption'].values[0])
...
#    return wealth_track, checked_age, initial_df, df
    return wealth_track, ct_track, utontine_track, ubequest
...
#    plt.plot(np.array(ct_track)*np.array(wealth_track[0]))
    ...
    plt.plot(list(range(50, 50+len(wealth_track))), np.array(ct_track)*np.array(wealth_track))
...
#    
#for i in range(10):    
#    plt.plot(list(range(50, 50+len(ct_total[i]))), ct_total[i])
#plt.legend()
...
plt.title('The monetary consumption for unhealthy male (size '+ str(num_of_people)+')')
plt.hlines(wealth_track[0]*ct_track[0], 50, 100,linestyles='dashed',label='theoretical',colors = 'indianred')
