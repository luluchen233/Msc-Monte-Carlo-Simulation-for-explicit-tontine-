...
#        df.iloc[i,4] = at
    ...
    ubequest_track = [df.loc[df['id']==1, 'bequest_motive'].values[0]*df.loc[df['id']==1, 'mortality rate'].values[0]*np.log((1-df.loc[df['id']==1, 'tontine proportion'].values[0]) * df.loc[df['id']==1, 'money_total'].values[0])]
...
#                ct_track.append(df.loc[df['id']==i,'optimal_consumption'].values[0])
        for i in range(1,2):        
            if df.loc[df['id']==1,'money_total'].empty==False:
                ...
                ubequest_track.append(df.loc[df['id']==1, 'bequest_motive'].values[0]*df.loc[df['id']==1, 'mortality rate'].values[0]*np.log((1-df.loc[df['id']==1, 'tontine proportion'].values[0]) * df.loc[df['id']==1, 'money_total'].values[0]))
#                ct_track.append(df.loc[df['id']==i,'optimal_consumption'].values[0])
...
for i in range(len(bequest_range)):
    ...
    #start_total = timeit.default_timer()
        wealth_track, ct_track, ubequest_track, utontine_track = wealth_monte_carlo(num_of_people, death_rate[:,run], b)
#        ax.plot(list(range(50, 50+len(wealth_track))), np.array(wealth_track) * np.array(ct_track), label='b='+str(b))
#        ax.plot(list(range(50, 50+len(wealth_track))), np.cumsum(ubequest_track), label='b='+str(b))
        ax.plot(list(range(50, 50+len(wealth_track))), np.cumsum(utontine_track), label='b='+str(b))
#        ax.plot(list(range(50, 50+len(wealth_track))), np.cumsum(np.array(ubequest_track)+np.array(utontine_track)), label='b='+str(b))
        
plt.xlabel("t=Age")
plt.grid()
#plt.ylabel("$c_t^{*}X_t$      ", rotation=0) 
#plt.ylabel("utility of tontine") 
...
#plt.title('The life long wealth for open tontine group ($\gamma=0$, Size = 500)')
#plt.title('The optimal monetary consumption ($\gamma=0$, Size = 500)')
plt.title('consumption utility ($\gamma=0$, Size = 500)')
#plt.title('Bequest account utility ($\gamma=0$, Size = 500)')
#plt.title('The life long utility ($\gamma=0$, Size = 500)')
plt.tight_layout()
