...
#%% initialization
def wealth_monte_carlo(num_of_people, death_rate_total):
    num_of_variables = 12
    np.random.seed(30)
...
for run in range(runtime):
    wealth = wealth_monte_carlo(num_of_people, death_rate[:,run])
    plt.plot(list(range(50, 50+len(wealth))), wealth)
    print('This is ', run, ' round')
    
plt.xlabel("t=Age")
plt.grid()
plt.ylabel("Wealth ($X_t$)")  
plt.title('Monte Carlo Simulation for closed tontine group with high randomness (Size ' + str(num_of_people) + ')')
